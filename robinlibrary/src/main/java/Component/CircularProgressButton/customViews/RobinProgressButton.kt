package Component.CircularProgressButton.customViews

import Component.CircularProgressButton.animatedDrawables.CircularProgressAnimatedDrawable
import Component.CircularProgressButton.animatedDrawables.CircularRevealAnimatedDrawable
import Component.CircularProgressButton.animatedDrawables.ProgressType
import Component.CircularProgressButton.presentation.State
import Component.CircularProgressButton.updateHeight
import Component.CircularProgressButton.updateWidth
import Component.CircularProgressButton.utils.addLifecycleObserver
...
import Component.CircularProgressButton.utils.parseGradientDrawable

interface RobinProgressButton : Drawable.Callback, LifecycleObserver {
...
internal fun RobinProgressButton.init(attrs: AttributeSet? = null, defStyleAttr: Int = 0) {
    ...
    val tempDrawable = typedArrayBg?.getDrawable(0) ?: ContextCompat.getDrawable(
        getContext(),
        R.drawable.shape_default
    )!!.let {
        when (it) {
...
internal fun RobinProgressButton.config(tArray: TypedArray) {
    ...
    spinningBarWidth =
        tArray.getDimension(R.styleable.CircularProgressButton_spinning_bar_width, 10f)
    spinningBarColor =
        tArray.getColor(R.styleable.CircularProgressButton_spinning_bar_color, spinningBarColor)
    paddingProgress =
        tArray.getDimension(R.styleable.CircularProgressButton_spinning_bar_padding, 0F)
}


internal fun RobinProgressButton.createProgressDrawable(): CircularProgressAnimatedDrawable =
...
internal fun RobinProgressButton.createRevealAnimatedDrawable(
    fillColor: Int,
    bitmap: Bitmap
...
internal fun cornerAnimator(drawable: Drawable, initial: Float, final: Float) =
    when (drawable) {
        is GradientDrawable -> ObjectAnimator.ofFloat(drawable, "cornerRadius", initial, final)
            ...
            parseGradientDrawable(drawable),
            "cornerRadius",
            initial,
            final
        )
